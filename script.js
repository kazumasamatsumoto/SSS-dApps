...
const address = symbol.Address.createFromRawAddress(
    ...
    .then((accountInfo) => {
      for (let m of accountInfo.mosaics) {
        if (m.id.id.toHex() === XYM_ID) {
          ...
          dom_xym.innerText = `XYM Balance : ${
            m.amount.compact() / Math.pow(10, 6)
          }`;
        }
      }
    });
  ...
  transactionHttp
    ...
    .then((txs) => {
      ...
      for (let tx of txs.data) {
        ...
        dom_txType.innerText = `Transaction Type : ${getTransactionType(
          tx.type
        )}`;
